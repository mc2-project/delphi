macro_rules! bigint_impl {
    ($name:ident, $num_limbs:expr, $iter_name:ident) => {
        ...
        impl Iterator for $iter_name {
            ...
            fn next(&mut self) -> Option<Self::Item> {
                let result = self.int.0.get(self.index).map(|&l| l);
                self.index += 1;
                result
            }

            fn size_hint(&self) -> (usize, Option<usize>) {
                let len = self.int.0.len();
                (len, Some(len))
            }
        }
        ...
        impl FromIterator<u64> for $name {
            ...
            #[inline]
            fn from_iter<I: IntoIterator<Item = u64>>(iter: I) -> $name {
                let mut cur = Self::default();
                for (i, limb) in iter.into_iter().enumerate() {
                    cur.0[i] = limb
                }
                cur
            }

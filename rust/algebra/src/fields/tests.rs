...
fn random_negation_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
...
fn random_addition_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    for _ in 0..ITERATIONS {
        let a = F::uniform(rng);
        let b = F::uniform(rng);
        let c = F::uniform(rng);
...
fn random_subtraction_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    for _ in 0..ITERATIONS {
        let a = F::uniform(rng);
        let b = F::uniform(rng);
...
fn random_multiplication_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    for _ in 0..ITERATIONS {
        let a = F::uniform(rng);
        let b = F::uniform(rng);
        let c = F::uniform(rng);
...
fn random_inversion_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    assert!(F::zero().inverse().is_none());

    for _ in 0..ITERATIONS {
        let mut a = F::uniform(rng);
...
fn random_doubling_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    for _ in 0..ITERATIONS {
        let mut a = F::uniform(rng);
        let mut b = a;
        a += &b;
...
fn random_squaring_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    for _ in 0..ITERATIONS {
        let mut a = F::uniform(rng);
        let mut b = a;
        a *= &b;
...
fn random_expansion_tests<F: Field, R: CryptoRng + RngCore>(rng: &mut R) {
    for _ in 0..ITERATIONS {
...
fn random_field_tests<F: Field>() {
    ...
    {
        let a = F::uniform(&mut rng) * &F::zero();
        assert!(a.is_zero());
    }

    // Addition by zero
    {
        let mut a = F::uniform(&mut rng);
        let copy = a;
        a += &F::zero();
        assert_eq!(a, copy);
    }
